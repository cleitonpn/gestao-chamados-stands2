...
// ---- Helpers ----
// Converts any value to a safe displayable string (avoids React error #31 on objects)
function toText(v) {
  ...
  if (t === "object") {
    // Event-like object normalization
    const maybeName = v.nome || v.name || v.titulo || v.title || v.evento || v.feira || v.cliente || v.projeto;
    if (maybeName) return String(maybeName);
    const di = v.dataInicio || v.inicio || v.startDate;
    const df = v.dataFim || v.fim || v.endDate;
    if (di || df) return `${formatDateBR(di)} → ${formatDateBR(df)}`;
                ...
                <dl className="grid grid-cols-1 gap-y-3">
                  {projectInfo.map(({ k, v }) => (
                    <div key={k} className="flex items-start justify-between gap-3">
                      ...
                      <dd className="text-sm font-medium text-neutral-800 text-right">{toText(v)}</dd>
                    </div>
                  ))}
                </dl>
