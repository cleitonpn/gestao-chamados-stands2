...
export default function TVPanel() {
  ...
  // SLA lista de violações (para rotação)
  ...
  useEffect(() => {
    const fit = () => {
      ...
      setScale(Math.max(0.6, Math.min(s, 1.25))); // limites
    };
    fit();
  ...
  useEffect(() => {
    ...
    // chamados (KPIs + feed + som de novos)
    const unsubTickets = onSnapshot(collection(db, 'chamados'), (snap) => {
      ...
      // SLA (violado e risco) + lista de violados
      const slaCfg = { baixa: 240, media: 24, alta: 12, urgente: 2 }; // horas
      let viol = 0, risk = 0;
      const violatedList = [];
      tickets.filter(t => !['concluido','cancelado','arquivado'].includes(t?.status)).forEach(t => {
        const h = slaCfg[t?.prioridade];
      ...
      setActivityFeed(sorted.slice(0, 20).map(t => {
        ...
        let message, icon;
        if (t.status === 'aberto')       { message = `Novo: "${t.titulo}"`;       icon = PlusCircle; }
        else if (['concluido','arquivado'].includes(t.status)) { message = `Finalizado: "${t.titulo}"`; icon = CheckCircle; }
        else if (t.status === 'executado_aguardando_validacao') { message = `Executado: "${t.titulo}"`;  icon = UserCheck; }
        else { message = `Atualizado: "${t.titulo}"`; icon = ArrowRightCircle; }
        return { id: t.id, status: t.status, message, icon, timeAgo: formatTimeAgo(ts) };
      }));

      // SOM: novos "aberto" após inicialização
      const openIdsNow = new Set(open.map(t => t.id));
      if (!initializedOpens.current) {
        // Primeira carga: não tocar som, apenas marcar vistos
        seenOpenIds.current = openIdsNow;
        initializedOpens.current = true;
      } else {
        let hasNew = false;
        openIdsNow.forEach(id => {
          if (!seenOpenIds.current.has(id)) { hasNew = true; }
        });
        if (hasNew) ping();
        seenOpenIds.current = openIdsNow;
      }
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setSlaView(v => (v === 'summary' ? 'areas' : 'summary'));
    }, 10000);
    return () => clearInterval(id);
  }, []);
  ...
  // Agrupamento de violações de SLA por área (para a view "areas")
