...
async function ensureUser() {
  ...
  return new Promise((resolve) => {
    const unsub = onAuthStateChanged(auth, (u) => { unsub(); resolve(u || null); }, () => { unsub(); resolve(null); });
  });
}
...
export default function AllDiariesPage() {
  ...
  // carrega projetos permitidos (apenas ativos)
  useEffect(() => {
    (async () => {
      const u = currentUser || (await ensureUser());
      ...
      const includesUid = (maybeList, id) => {
        if (!id || !maybeList) return false;
        if (Array.isArray(maybeList)) return maybeList.includes(id);
        if (typeof maybeList === "object") return id in maybeList;
        return maybeList === id;
      };
      ...
      snap.forEach((d) => {
        ...
        const privileged = ["administrador", "gerente", "operador"].includes(role);
        const isMine =
  ...
  const loadFeed = async () => {
    ...
    try {
      ...
      if (selectedProjectId) {
        res = await diaryService.fetchFeedByProject({ projectId: selectedProjectId, pageSize: limitN });
      } else {
  ...
  useEffect(() => { loadFeed(); /* eslint-disable-next-line */ }, [selectedProjectId, limitN]);
  ...
  return (
    <div className="px-4 md:px-6 py-4 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        {highlights.length === 0 && <div className="md:col-span-3 text-sm text-slate-500">Nenhum diário recente.</div>}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-4">
            ...
            <DiaryForm projects={projects} onSubmit={handleCreate} defaultProjectId={selectedProjectId || ""} disabled={!authReady} />
            {!authReady && <p className="mt-2 text-xs text-slate-500">Carregando usuário…</p>}
          </div>
        </div>
              ...
              <div className="flex items-center justify-between mb-2 p-2 rounded-md bg-slate-50 border border-slate-200">
                ...
                <button
                  ...
                  className={`text-xs px-3 py-1 rounded-md ${
                    selectedIds.size === 0 ? "bg-slate-100 text-slate-400" : "bg-red-600 text-white hover:bg-red-500"
                  }`}
                >
        ...
        {/* direita: projetos ativos permitidos */}
        <div className="lg:col-span-1">
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-4">
            ...
            <div className="mt-3 h-[60vh] overflow-y-auto pr-1">
              {projects.length === 0 && <div className="text-sm text-slate-500">Nenhum projeto encontrado.</div>}
